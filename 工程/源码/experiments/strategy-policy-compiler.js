...
function stageCarryName(stage) {
  if (!stage) return "";
  if (typeof stage.carry === "string") return stage.carry;
  return stage.carryName
    || stage.carry && (stage.carry.itemHolder || stage.carry.execution || stage.carry.strategic)
    || "";
}
...
function compileStage({ row, template, stage, profile, opponents, unitTraits }) {
  ...
  const stageHolder = stageCarryName(stage);
  const holder = stageHolder === target ? alternateHolder(stage, target, row.carryItems || []) : stageHolder;
  ...
  if (!targetPresent) {
    return {
      ...
      baselineHolder: stageHolder,
      targetPresent: false,
      targetStarThreshold: null,
      certified: false,
      gate,
      inferenceByStar: [],
      text: `${stage.levelMax}级前${target}不在阶段棋盘，继续${stageHolder}代持`,
    };
  }
...
function run() {
  ...
  assert(plans.every(plan => typeof plan.baselineCarry === "string" && typeof plan.currentHolder === "string"
    && plan.stages.every(stage => typeof stage.baselineHolder === "string")
    && plan.holderRules.every(rule => typeof rule.holder === "string" && typeof rule.fromCarry === "string")),
  "策略编译器的持装者与对照主C必须归一化为名称字符串");
