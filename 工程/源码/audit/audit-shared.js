const fs = require('fs');
const { currentVersion } = require('../core/version-context');
const { ROOT, dataPath } = require('../lib/project-paths');

const readJSON = p => JSON.parse(fs.readFileSync(dataPath(p), 'utf8'));

const data = {
  chess: readJSON('chess.js').data,
  equip: readJSON('equip.js').data,
  race: readJSON('race.js').data,
  job: readJSON('job.js').data,
  hex: readJSON('hex.js').data,
  monster: readJSON('monster.js').data,
  lineups: readJSON('lineup_detail_total.json').lineup_list,
};

const VERSION = readJSON('chess.js').version;
