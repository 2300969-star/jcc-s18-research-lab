...
const { simulate } = require('../core/model.js');
...
const chess = JSON.parse(fs.readFileSync(dataPath('chess.js'), 'utf8')).data;
...
console.log(resultOnly
  ? '输出: 生成产物/结果/meta_discovery_results.json（result-only）'
  : '输出: 生成产物/结果/meta_discovery_results.json, 文档/研究报告/元阵容自动求解.md, 生成产物/结果/model_results.json');
