...
const data = {
  chess: read('资源/游戏数据/chess.js').data,
  equip: read('资源/游戏数据/equip.js').data,
  hex: read('资源/游戏数据/hex.js').data,
  race: read('资源/游戏数据/race.js').data,
  job: read('资源/游戏数据/job.js').data,
  lineups: read('资源/游戏数据/lineup_detail_total.json').lineup_list,
};
...
function officialTemplates() {
  return data.lineups.map(lineup => {
    ...
    const template = {
      ...
      augmentPrefs,
      augmentCats,
      monsterPrefs: uniq([...csv(detail.monster && detail.monster.recomm), ...csv(detail.monster && detail.monster.replace)].map(id => (read('资源/游戏数据/monster.js').data[id] || {}).name || id)),
...
const out = {
  version: read('资源/游戏数据/chess.js').version + '-' + read('资源/游戏数据/chess.js').season,
...
console.log(process.env.STAGE2_WRITE_REPORT === '0'
  ? '输出: 生成产物/结果/stage2_matcher_results.json, 应用/前端/stage2-matcher-data.js'
  : '输出: 生成产物/结果/stage2_matcher_results.json, 应用/前端/stage2-matcher-data.js, 文档/研究报告/二阶段交互匹配器.md');
