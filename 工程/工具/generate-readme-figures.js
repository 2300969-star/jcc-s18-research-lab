...
const RESULTS = path.join(ROOT, "生成产物", "结果");
const OUTPUT = path.join(ROOT, "文档", "图片", "research");
const WIDTH = 1200;
const HEIGHT = 680;
const C = {
  ink: "#172033",
...
function save(name, content) {
  ...
  console.log(`generated 文档/图片/research/${name}`);
}
...
function figureAssetValue() {
  ...
  save("formula-asset-value.svg", frame(
    ...
    body,
    "口径：应用/前端/matcher-core.js fallbackWeights 与确定性算子常量"
  ));
}

function loadOdds() {
  const context = { globalThis: {} };
  vm.runInNewContext(fs.readFileSync(path.join(ROOT, "应用", "前端", "odds-data.js"), "utf8"), context);
...
function figureShopOdds() {
  ...
  save("formula-shop-odds.svg", frame(
    ...
    body,
    "数据：应用/前端/odds-data.js；概率来源注释见该文件，牌池份数按 S8 同口径"
  ));
}
