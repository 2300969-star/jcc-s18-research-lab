...
function writeReport(data) {
  ...
  fs.writeFileSync(path.join(ROOT, 'reports', '科研驾驶舱.md'), lines.join('\n'));
}

fs.writeFileSync(path.join(ROOT, 'research_lab_results.json'), JSON.stringify(out, null, 1));
fs.writeFileSync(path.join(ROOT, '可视化', 'research-lab-data.js'), 'window.RESEARCH_LAB=' + JSON.stringify(out) + ';');
writeReport(out);

console.log('科研驾驶舱生成完成');
console.log(out.thesis.currentTheory);
out.hypotheses.forEach(h => console.log(`${h.id} [${h.status}] ${h.title}`));
console.log('输出: research_lab_results.json, 可视化/research-lab-data.js, reports/科研驾驶舱.md');
