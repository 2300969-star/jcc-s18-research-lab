...
const read = file => JSON.parse(fs.readFileSync(path.join(ROOT, 'data', file), 'utf8')).data;
...
function report(data) {
  ...
  fs.writeFileSync(path.join(ROOT, 'reports', '姐妹无限金克丝实验.md'), lines.join('\n'));
}

fs.writeFileSync(path.join(ROOT, 'jinx_sisters_results.json'), JSON.stringify(out, null, 1));
report(out);
