...
function writeReport(dataOut) {
  ...
  fs.writeFileSync(path.join(ROOT, 'reports', '二阶段交互匹配器.md'), lines.join('\n'));
}

fs.writeFileSync(path.join(ROOT, 'stage2_matcher_results.json'), JSON.stringify(out, null, 1));
fs.writeFileSync(path.join(ROOT, '可视化', 'stage2-matcher-data.js'), 'window.STAGE2_MATCHER=' + JSON.stringify(out) + ';');
writeReport(out);

console.log('二阶段匹配器数据生成完成');
out.examples.forEach(ex => console.log(`${ex.name}: ${ex.result.map(r => r.template + '(' + r.score + ')').join(' / ')}`));
console.log('输出: stage2_matcher_results.json, 可视化/stage2-matcher-data.js, reports/二阶段交互匹配器.md');
