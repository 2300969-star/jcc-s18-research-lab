...
const job = D('job.js');
const transitions = JSON.parse(fs.readFileSync(path.join(ROOT, 'transition_results.json'), 'utf8'));
const discovery = JSON.parse(fs.readFileSync(path.join(ROOT, 'discovery_results.json'), 'utf8'));
...
function writeReport(out) {
  ...
  fs.writeFileSync(path.join(ROOT, 'reports', '数值透视.md'), lines.join('\n'));
}
...
fs.writeFileSync(path.join(ROOT, 'numeric_lens_results.json'), JSON.stringify(out, null, 1));
fs.writeFileSync(path.join(ROOT, '可视化', 'numeric-lens-data.js'), 'window.NUMERIC_LENS=' + JSON.stringify(out) + ';');
writeReport(out);

console.log('数值透视完成');
out.routes.slice(0, 5).forEach((r, i) => console.log(`${i + 1}. ${r.name}: ${r.keyFindings.join('；')}`));
console.log('输出: numeric_lens_results.json, 可视化/numeric-lens-data.js, reports/数值透视.md');
