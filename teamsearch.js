...
const race = JSON.parse(fs.readFileSync(path.join(__dirname, 'data/race.js'))).data;
const job = JSON.parse(fs.readFileSync(path.join(__dirname, 'data/job.js'))).data;
...
const TFX = {
  ...
  164: { tiers: [3], fx: () => ({ team: { ampPct: 25 } }) },                                                     // 超级英雄(含1个3星+5%)
  ...
  123: { tiers: [2, 4, 6, 8], fx: t => ({ team: { ap: [25, 50, 80, 120][t] } }) },                               // 灵能使
  124: { tiers: [2, 4, 6, 8], fx: t => ({ member: { asPct: [30, 54, 90, 144][t] }, util: [0, 0, 1, 2][t] }) },   // 决斗大师(按均值6层折算)
  125: { tiers: [2, 3, 4, 5], fx: t => ({ team: { adPct: [5, 10, 15, 20][t] } }) },                              // 强袭枪手
