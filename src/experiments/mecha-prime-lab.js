...
const fs = require('fs');
const { simulate, chess, equip } = require('../core/model.js');
const { currentVersion } = require('../core/version-context');
const { ROOT, resultPath, reportPath, ensureOutputDirs } = require('../lib/project-paths');

const OUT_JSON = resultPath('mecha_prime_results.json');
const OUT_REPORT = reportPath('至尊机甲实验.md');
...
function read(file) {
  return JSON.parse(fs.readFileSync(resultPath(file), 'utf8'));
}
...
function main() {
  ...
  console.log('输出: artifacts/results/mecha_prime_results.json, docs/reports/至尊机甲实验.md');
}
