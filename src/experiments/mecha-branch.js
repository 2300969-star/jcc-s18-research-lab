...
const { ROOT, dataPath, resultPath, reportPath, publicPath, ensureOutputDirs } = require('../lib/project-paths');

const version = currentVersion(ROOT);
const readData = p => JSON.parse(fs.readFileSync(dataPath(p), 'utf8')).data;
...
function writeReport(out) {
  ...
  ensureOutputDirs();
  fs.writeFileSync(reportPath('机甲分叉实验.md'), lines.join('\n'));
}
...
fs.writeFileSync(resultPath('mecha_branch_results.json'), JSON.stringify(out, null, 1));
fs.writeFileSync(publicPath('mecha-branch-data.js'), 'window.MECHA_BRANCH=' + JSON.stringify(out) + ';');
writeReport(out);
...
console.log('输出: artifacts/results/mecha_branch_results.json, public/mecha-branch-data.js, docs/reports/机甲分叉实验.md');
