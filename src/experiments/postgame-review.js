...
const lens = JSON.parse(fs.readFileSync(resultPath('numeric_lens_results.json'), 'utf8'));
...
function writeReport(data) {
  ...
  ensureOutputDirs();
  fs.writeFileSync(reportPath('贾克斯两局实战校准.md'), lines.join('\n'));
}

fs.writeFileSync(resultPath('postgame_review.json'), JSON.stringify(out, null, 1));
writeReport(out);
...
console.log('输出: artifacts/results/postgame_review.json, docs/reports/贾克斯两局实战校准.md');
