...
const augmentOptions = new Map(((stage2.options || {}).augmentSearch || []).map(x => [x.name, x]));

function numericValues(text) {
  return [...String(text || '').matchAll(/(\d+(?:\.\d+)?)%?/g)].map(x => Number(x[1])).filter(Number.isFinite);
}

function genericConditionFit(t, category, option) {
  const f = features.get(t.id);
  const text = `${option.name || ''} ${option.desc || ''}`;
  const nums = numericValues(text).filter(x => x <= 100);
  let gain = 4;
  const reasons = [];
  if (category === '战力') {
    const pctGain = Math.min(12, (Math.max(0, ...nums) || 0) * 0.28);
    gain += pctGain;
    if (/法力|回蓝|施放/.test(text) && f.direction === 'magic') { gain += 6; reasons.push('回蓝命中法系施法轴'); }
    if (/吸血|治疗|护盾|减伤|12秒|飞升/.test(text) && f.frontline >= 2) { gain += 5; reasons.push('前排支持持续战触发'); }
    if (pctGain) reasons.push(`文本数值折算+${round(pctGain)}适配`);
  } else if (category === '经济') {
    if (/刷新|DD街区|快速思考|门票/.test(text)) {
      const value = f.starTarget >= 3 ? 12 : 6;
      gain += value; reasons.push(`刷新经济适配${f.carry.name}${f.starTarget}星`);
    }
    if (/复制器|团队建设|英勇福袋|升星/.test(text) && f.cost <= 3) {
      gain += 11; reasons.push('低费追三可直接减少同名牌缺口');
    }
    if (/高端购物|升级咯|经验|后期专家/.test(text) && f.cost >= 4) {
      gain += 12; reasons.push(`高费主C${f.carry.name}受益于等级概率`);
    }
    if (/金币|利息|对冲基金|利滚利/.test(text)) {
      const gold = Math.min(8, (Math.max(0, ...nums) || 0) / 5);
      gain += gold; reasons.push('金币折算提升成型可达性');
    }
  } else if (category === '装备') {
    const bottleneck = round((1 - f.itemFlex) * 10);
    gain += 7 + bottleneck;
    reasons.push(`装备弹性${Math.round(f.itemFlex * 100)}%，缓解瓶颈+${bottleneck}`);
    if (/神器|光明|锻炉/.test(text)) { gain += 4; reasons.push('高上限装备提供额外主C/前排期权'); }
  }
  return { gain: round(clamp(gain, 2, 22)), reasons: reasons.length ? reasons : ['命中路线推荐符文位'] };
}

function conditionCategories(option) {
  const text = `${option.name || ''} ${option.desc || ''}`;
  if (!option.name || /^\d+$/.test(option.name)) return [];
  if (/小巨人|小小英雄回复|玩家生命值|金币|利息|刷新|经验|升级|复制器|高端购物|后期专家|DD街区|明智消费|团队建设|英勇福袋/.test(text)) return ['经济'];
  return (option.cats || []).filter(cat => ['战力', '经济', '装备'].includes(cat));
}

const heroConditions = templates.filter(t => t.mechanic && byId.get(t.id)).map(t => {
  const cert = byId.get(t.id);
  const gates = [];
  if (t.mechanic.requiredAugment) gates.push({ kind: '符文', value: t.mechanic.requiredAugment });
  if ((t.mechanic.requiredUnits || []).length) gates.push({ kind: '棋子', value: t.mechanic.requiredUnits.join('+') });
  return {
    category: '英雄专属',
    templateId: t.id,
    templateName: t.name,
    conditionScore: cert.metrics.robustness,
    fitGain: 0,
    evidenceLevel: cert.evidenceLevel,
    metrics: cert.metrics,
    gates,
    reasons: ['专属机制按有/无强化反事实差分生成'],
  };
});
...
templates.forEach(t => {
  const cert = byId.get(t.id);
  if (!cert) return;
  uniq(t.augmentPrefs || []).forEach(name => {
    const option = augmentOptions.get(name);
    if (!option || (option.cats || []).includes('英雄强化')) return;
    conditionCategories(option).forEach(category => {
      const fit = genericConditionFit(t, category, option);
      // 普通符文只能补强路线，不能把基础稳健度线性推到满分；越接近100，边际提升越小。
      const liftRatio = 0.65 * fit.gain / 22;
      const conditionScore = round(cert.metrics.robustness + (100 - cert.metrics.robustness) * liftRatio);
      genericConditions.push({
        category,
        templateId: t.id,
        templateName: t.name,
        conditionScore,
        conditionLift: round(conditionScore - cert.metrics.robustness),
        fitGain: fit.gain,
        evidenceLevel: cert.evidenceLevel,
        metrics: cert.metrics,
        gates: [{ kind: '符文', value: name }],
        reasons: fit.reasons,
      });
    });
  });
...
function topConditions(rows, limit = 24) {
  ...
  return rows.sort((a, b) => b.conditionScore - a.conditionScore || b.metrics.cvar10 - a.metrics.cvar10)
    .filter(x => {
      const key = `${x.category}|${x.templateId}`;
...
const conditionRankings = {
  英雄专属: topConditions(heroConditions),
  战力: topConditions(genericConditions.filter(x => x.category === '战力')),
  经济: topConditions(genericConditions.filter(x => x.category === '经济')),
  装备: topConditions(genericConditions.filter(x => x.category === '装备')),
};
...
ensureOutputDirs();
fs.writeFileSync(resultPath('route_certification_results.json'), JSON.stringify(result, null, 1));
...
fs.writeFileSync(resultPath('meta_discovery_results.json'), JSON.stringify(meta, null, 1));
...
const report = [
  ...
  '## 普通符文条件适配', '',
  ...['战力', '经济', '装备'].flatMap(category => [
    `### ${category}`,
    ...conditionRankings[category].slice(0, 8).map((x, i) => `${i + 1}. ${x.gates.map(g => `${g.kind}:${g.value}`).join('；')} → ${x.templateName}：稳健${x.metrics.robustness}→条件适配${x.conditionScore}（边际+${x.conditionLift}）；${x.reasons.join('；')}`),
    '',
  ]),
  '## 认证边界', '',
  ...result.limitations.map(x => `- ${x}`),
];
fs.writeFileSync(reportPath('版本路线认证实验.md'), report.join('\n'));
...
console.log('输出: artifacts/results/route_certification_results.json, docs/reports/版本路线认证实验.md');
