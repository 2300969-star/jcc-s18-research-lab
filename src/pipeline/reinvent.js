...
const fs = require('fs');
const { simulate, itemStats, chess, equip } = require('../core/model.js');
const { currentVersion, currentLineups } = require('../core/version-context.js');
const { ROOT, dataPath, resultPath, publicPath, ensureOutputDirs } = require('../lib/project-paths');

const read = file => JSON.parse(fs.readFileSync(dataPath(file), 'utf8'));
const data = {
  hex: read('hex.js').data,
  race: read('race.js').data,
  job: read('job.js').data,
  lineups: read('lineup_detail_total.json').lineup_list,
};
...
ensureOutputDirs();
fs.writeFileSync(resultPath('reinvent_results.json'), JSON.stringify(out, null, 1));
fs.writeFileSync(publicPath('reinvent-data.js'), 'window.REINVENT=' + JSON.stringify(out) + ';');
