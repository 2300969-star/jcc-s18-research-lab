...
const D = JSON.parse(fs.readFileSync(resultPath('discovery_results.json'), 'utf8'));
let POSTGAME = null;
try {
  POSTGAME = JSON.parse(fs.readFileSync(resultPath('postgame_review.json'), 'utf8'));
} catch (_) {
  POSTGAME = null;
}
...
function writeReport(data) {
  ...
  ensureOutputDirs();
  fs.writeFileSync(reportPath('平滑过渡阵容路线.md'), lines.join('\n'));
}

fs.writeFileSync(resultPath('transition_results.json'), JSON.stringify(out, null, 1));
fs.writeFileSync(publicPath('transition-data.js'), 'window.TRANSITIONS=' + JSON.stringify(out) + ';');
writeReport(out);

console.log('平滑过渡路线生成完成');
out.routes.slice(0, 8).forEach((r, i) => console.log(`${i + 1}. [${r.score}] ${r.name} | ${r.verdict}`));
console.log('输出: artifacts/results/transition_results.json, public/transition-data.js, docs/reports/平滑过渡阵容路线.md');
