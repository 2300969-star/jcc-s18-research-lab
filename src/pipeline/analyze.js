// 解析 jcc.qq.com 怪兽入侵(mode8 S18) 官方阵容数据 → docs/notes/阵容全量明细.md + 聚合统计
const fs = require('fs');
const { currentVersion, currentLineups } = require('../core/version-context.js');
const { ROOT, dataPath, notePath, ensureOutputDirs } = require('../lib/project-paths');
const D = p => JSON.parse(fs.readFileSync(dataPath(p), 'utf8'));
...
const Q = { S: 0, A: 1, B: 2 };
const list = currentLineups(lineup_list, ROOT)
...
ensureOutputDirs();
fs.writeFileSync(notePath('阵容全量明细.md'), md);
