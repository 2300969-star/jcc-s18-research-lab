...
function main() {
  ...
  ensureOutputDirs();
  fs.writeFileSync(resultPath('audit_results.json'), JSON.stringify(results, null, 2));
  fs.writeFileSync(reportPath('数值与Bug审计.md'), buildReport(results));
  fs.mkdirSync(publicPath('audit'), { recursive: true });
  fs.writeFileSync(publicPath('audit', 'data.js'), 'window.AUDIT=' + JSON.stringify(results) + ';');
  fs.writeFileSync(publicPath('audit-data.js'), 'window.AUDIT=' + JSON.stringify(results) + ';');

  console.log('审计完成: audit_results.json');
  console.log('审计报告: docs/reports/数值与Bug审计.md');
  console.log('审计前端数据: public/audit/data.js');
